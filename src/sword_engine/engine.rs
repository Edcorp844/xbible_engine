...
use crate::ffi::*;
use super::module::{ModuleBook, ModuleChapter, SwordModule};
...
impl SwordEngine {
    ...
    pub fn lookup_verse(&self, module_name: &str, reference: &str) -> String {
        let inner = self.inner.lock().unwrap();
        let c_mod_name = CString::new(module_name).unwrap();
        let c_reference = CString::new(reference).unwrap();

        unsafe {
            let h_module = org_crosswire_sword_SWMgr_getModuleByName(inner.mgr, c_mod_name.as_ptr());
            if h_module == 0 {
                return format!("Module '{}' not found", module_name);
            }

            // Set the key (verse reference)
            org_crosswire_sword_SWModule_setKeyText(h_module, c_reference.as_ptr());

            // Render the text
            let text_ptr = org_crosswire_sword_SWModule_renderText(h_module);
            if text_ptr.is_null() {
                String::new()
            } else {
    ...
    pub fn lookup_verse_stripped(&self, module_name: &str, reference: &str) -> String {
        let inner = self.inner.lock().unwrap();
        let c_mod_name = CString::new(module_name).unwrap();
        let c_reference = CString::new(reference).unwrap();
        ...
        unsafe {
            ...
            // Strip markup from the text
            let text_ptr = org_crosswire_sword_SWModule_stripText(h_module);
            if text_ptr.is_null() {
                String::new()
            } else {
                CStr::from_ptr(text_ptr).to_string_lossy().into_owned()
    ...
    fn from_code(&self, code: &str) -> String {
        ...
        match code {
            ...
            "el" => "Greek",
            "la" => "Latin",
            _ => code, // Return the code if not recognized
        }
        .to_string()
    }
