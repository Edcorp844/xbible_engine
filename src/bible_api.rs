...
impl XBibleEngine {
    ...
    pub fn get_available_categories(&self) -> Vec<String> {
        let mut categories: Vec<String> = self.module_engine.get_modules().into_iter().map(|m| m.category).collect();
        categories.sort();
        categories.dedup();
        categories
    }
    ...
    pub fn get_cult_modules(&self) -> Vec<SwordModule> {
        self.module_engine.get_modules_by_category(vec!["Cults / Unorthodox / Questionable Material"])
    }
    ...
    pub fn set_global_options(&self, options: Vec<EngineGlobalOption>) {
        unsafe {
            options.iter().for_each(|opt| {
                self.module_engine.set_global_options(
                    &[opt.name.as_str()],
                    &opt.state.as_str(),
                )
            });
    ...
    pub fn get_content(&self, module_name: &str, reference: &str) -> Vec<Section> {
        ...
        if let Some(module) = modules.into_iter().find(|m| m.name == module_name) {
            self.module_engine.get_single_entry(Some(&module), reference)
        } else {
            Vec::new()
        }
    }
    ...
    pub fn install_module(&self, source: &str, module_name: &str) -> i32 {
        self.module_engine.install_remote_module(source, module_name)
    }
